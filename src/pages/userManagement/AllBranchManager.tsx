...
const AllBranchManager = () => {
  const { data, isLoading } = useGetAllBranchManagersQuery("");
  const [deleteBranchManager] = useDeleteBranchManagerMutation();

  const handleDelete = async (id: string) => {
    const toastId = toast.loading("Deleting...");
    const res = (await deleteBranchManager(id)) as any;

    if (res?.data?.statusCode === 200) {
      toast.success("Branch Manager deleted successfully", { id: toastId });
    } else {
  ...
  return (
    <div>
      <h2 className="text-xl font-bold text-center my-5">All Branch Manager</h2>
